...
const PodiumDisplay = ({
  ...
  // Überprüfung, ob topVolunteers vorhanden und nicht leer ist
  if (!topVolunteers || topVolunteers.length === 0) {
    return (
        <div className="w-full bg-gray-50 p-6 rounded-lg h-[400px] flex items-center justify-center">
          <p className="text-gray-500">Noch keine Volunteers verfügbar</p>
        </div>
    );
  }

  // Überprüfung, ob genügend Volunteers vorhanden sind
  if (topVolunteers.length < 3) {
    return (
        <div className="w-full bg-gray-50 p-6 rounded-lg h-[400px] flex items-center justify-center">
          <p className="text-gray-500">Nicht genügend Volunteers für ein Podium</p>
        </div>
    );
  }
